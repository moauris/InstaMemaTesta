class Game {
    ...
    initRound(difficulty : number = 4) {
        ...
        let lower : number = Game.randLowerBounds[this.difficulty - 4];
        let upper : number = Game.randUpperBounds[this.difficulty - 4];

        let comboIndex : number = this.GetRandom(lower, 
            upper);

        let comboBin : number = Game.randomRollTable[comboIndex];

        this.GuessNumbers = 
            this.getNumArrFromBinary(comboBin, this.difficulty === 5);
        
        this.numbersGuessed = new Array<boolean>(this.difficulty);
    ...
    private static randLowerBounds : Array<number> = [0,   0,   126, 210, 246, 255];
    private static randUpperBounds : Array<number> = [125, 125, 209, 245, 254, 256];
    private static randomRollTable : Array<number> = [
